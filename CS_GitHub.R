...
ui=round((0.124*(-0.096472+(1.96*0.044128)))*100,2),pch=16,xlim=c(0.5,7.5),ylim=c(-2.5,1),xaxt='n',xlab='Model',ylab="% change in pop. growth rate",col="orange")
abline(h=-1,lty=3)
abline(h=0,lty=)
...
# Dassen from Sherley et al. 2019 corrected - no interaction
plotrix::plotCI(x=3.75,y=round((0.124*-0.0760497)*100,2),
                li=round((0.124*(-0.0760497-(1.96*0.0156372)))*100,2),
                ui=round((0.124*(-0.0760497+(1.96*0.0156372)))*100,2),add=T,pch=16,col="orange")

# Robben from Sherley et al. 2019 corrected - no interaction
plotrix::plotCI(x=4.25,y=round((0.124*-0.07867816)*100,2),
li=round((0.124*(-0.07867816-(1.96*0.0156488)))*100,2),
ui=round((0.124*(-0.07867816+(1.96*0.0156488)))*100,2),add=T,pch=16,col='purple')

# Dassen from Sherley et al. 2019 corrected with interaction
plotrix::plotCI(x=4.75,y=round((0.124*-0.09551272)*100,2),
                li=round((0.124*(-0.09551272-(1.96*0.0389522)))*100,2),
                ui=round((0.124*(-0.09551272+(1.96*0.0389522)))*100,2),add=T,pch=16,col="orange")

# Robben from Sherley et al. 2019 corrected with interaction
plotrix::plotCI(x=5.25,y=round((0.124*-0.05861958)*100,2),
                li=round((0.124*(-0.05861958-(1.96*0.03997866)))*100,2),
                ui=round((0.124*(-0.05861958+(1.96*0.03997866)))*100,2),add=T,pch=16,col='purple')


# Dassen from OLSPS 2021 (Gibbs sampling)
plotrix::plotCI(x=5.75,y=-0.942,li=-1.761,ui=-0.025,add=T,pch=16,col="orange")
# Robben from OLSPS 2021 (Gibbs sampling)
plotrix::plotCI(x=6.25,y=-0.397,li=-1.240,ui=0.620,add=T,pch=16,col='purple')


# Raw (unmodelled) mean differences
# Dassen
points(x=6.75,y=round((0.124*-0.0665567)*100,2),pch=3,col="orange",lwd=2)
# Robben
points(x=7.25,y=round((0.124*-0.07796)*100,2),pch=3,col="purple",lwd=2)

axis(1,at=1:7,labels=c("1.A_I*C_Y","2.A_I+C_Y","3.A_C_Y","4.SHE_I+C_YN","5.SHE_I*C_YN","6.OLS_I*C_Y","7.Raw means"),cex.axis=0.7)
